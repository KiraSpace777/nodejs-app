...
бібліотекою mailgun: функціонал відправлення листів

### // npm install nodemailer

бібліотекою nodemailer: функціонал відправлення листів

### // npm i jsonwebtoken

бібліотеку для роботи з токен JWT

### // npm i handlebars

популярний шаблонізатор для JavaScript (для створення листів/імейлів, з гарним форматуванням та певною логікою, повторюваними блоками, замість звичайного шаблонного рядку)
...
# ==== MODULE 5 (Пошта та зображення) ====

## Електронна пошта

На відміну від HTTP, у світі пошти існують спеціальні протоколи. Найпоширеніші з них:
SMTP (Simple Mail Transfer Protocol) — протокол відправлення пошти.
POP3 (Post Office Protocol v3) — протокол отримання пошти.

### SMTP — відправка листів

SMTP відповідає за доставку електронних повідомлень від відправника до поштового сервера отримувача.

Схема роботи:
Клієнт (наш додаток) надсилає лист через SMTP-сервер.
SMTP-сервер відправника встановлює з’єднання з сервером одержувача.
Лист передається на сервер одержувача.
Сервер одержувача зберігає лист у поштовій скриньці отримувача.
У нашому випадку ми будемо користуватись саме SMTP, оскільки в додатку найчастіше потрібно відправляти повідомлення, а не читати їх.

### POP3 — отримання листів

POP3 використовується для завантаження пошти із сервера на локальний клієнт (наприклад, у поштові програми).

Схема роботи:
Клієнт підключається до поштового сервера через POP3.
Усі повідомлення завантажуються на пристрій користувача.
За замовчуванням повідомлення видаляються з сервера (але це можна змінити в налаштуваннях).

### Відмінності SMTP і POP3

#### Напрям роботи:

SMTP — для відправлення.
POP3 — для отримання.

#### Зберігання повідомлень:

SMTP — не зберігає листи (тільки передає).
POP3 — дозволяє зберігати їх локально.

#### Шифрування:

Як і HTTP має HTTPS, так і SMTP/POP3 можуть використовувати TLS/SSL для захисту даних при передачі.

Таким чином, у реальному житті SMTP і POP3 працюють разом: один протокол відправляє листи, інший — забирає їх для отримувача.
А ми з вами зосередимось на найчастішому завданні у Node.js-додатках — надсиланні електронних листів через SMTP.

## Планування скидання паролю

Скидання паролю — це критично важливий функціонал у будь-якому додатку. Якщо користувач втратив доступ до свого паролю, ми маємо надати йому безпечний спосіб відновлення.
Але є проблема: система не може бути впевнена, що людина, яка просить змінити пароль, дійсно є власником акаунта. Тому нам потрібне стороннє джерело ідентифікації, якому ми можемо довіряти. У більшості випадків цим джерелом є електронна пошта: якщо користувач має доступ до свого email, ми можемо вважати, що це справді він.

#### Як це працює в реальних системах

Користувач вводить email у формі “Забули пароль?”.
Ми генеруємо спеціальний ідентифікатор і відправляємо його на пошту.
Користувач відкриває лист, переходить за посиланням і потрапляє на сторінку введення нового паролю.
Система перевіряє, чи ідентифікатор дійсний, і дозволяє змінити пароль.

#### Варіанти ідентифікації користувача

1. OTP (One-Time Password) — короткий одноразовий код, який користувач має ввести вручну.
2. Посилання з токеном — більш зручний підхід: ми кодуємо потрібну інформацію в посилання і відправляємо його на пошту. При кліку користувач потрапляє на сторінку відновлення паролю без введення додаткових кодів.

## Посилання з токеном (JWT-токен)

### Що таке JWT?

JWT (JSON Web Token) — це компактний токен, який дозволяє безпечно передавати дані між сторонами. Його стандарт описано у RFC 7519.
JWT складається з трьох частин, розділених крапками:
header.payload.signature

Header (заголовок) – містить метаінформацію: тип токена (JWT) і алгоритм підпису (HS256, RS256 тощо).
Payload (корисне навантаження) – у цій частині зберігаються дані (claims). Наприклад ідентифікатор користувача, дата видачі, час закінчення дії.
Signature (підпис) – створюється шляхом підпису header + payload секретним ключем. Це гарантує, що токен не можна підробити.

### Основні властивості JWT

Формат — звичайний рядок, зручний для передачі в URL або заголовках.
Самодостатність — у токені зберігається вся потрібна інформація для перевірки. Серверу не треба вести додаткову таблицю “активних токенів”.
Верифікація — сервер може перевірити справжність токена завдяки підпису.
Прозорість — будь-хто може розкодувати payload (він закодований у Base64). Тому ніколи не зберігайте там чутливу інформацію (наприклад, паролі).

### Приклад JWT

У цьому токені зберігається:
...
Вміст токена можна подивитись у зручному вигляді на сайті jwt.io.

### Наш план

#### 1. Створити ендпоінт POST /auth/request-reset-email, який:

перевірить email користувача,
згенерує JWT-токен,
відправить лист із посиланням для скидання паролю.

#### 2. Створити ендпоінт POST /auth/reset-password, який:

прийматиме токен і новий пароль,
перевірятиме токен,
оновлюватиме пароль користувача.

## Зображення у веб-додатках

У сучасних веб-додатках робота із зображеннями має велике значення, адже графічний контент привертає увагу користувачів і робить взаємодію з додатком приємнішою та ефективнішою. Нижче наведено ключові аспекти роботи з картинками:

#### 1. Завантаження зображень:

Користувачі повинні мати можливість завантажувати власні файли. Це включає вибір картинки на комп’ютері чи телефоні, передачу її на сервер і збереження шляху або посилання в базі даних.

#### 2. Зберігання та керування:

Файли можна зберігати локально на сервері або у зовнішніх сховищах (Amazon S3, Google Cloud Storage). Для великої кількості картинок застосовують бази даних чи сервіси управління контентом. Ми з вами розглянемо обидва варіанти: локальне збереження та завантаження у хмару.

#### 3. Оптимізація:

Щоб сторінки відкривалися швидко, зображення потрібно оптимізувати — стискати, зменшувати розмір, конвертувати у сучасні формати (наприклад, WebP) і за потреби підключати lazy loading. Багато хмарних сервісів надають інструменти оптимізації «з коробки».

#### 4. Масштабування та адаптивність:

Важливо мати можливість контролювати розміри й вигляд картинок: масштабувати, обрізати чи адаптувати їх під різні пристрої.

#### 5. Кешування та CDN:

Використання кешування та CDN (Content Delivery Network) значно пришвидшує доставку картинок користувачам по всьому світу. Це особливо актуально для важких зображень та застосунків із великою аудиторією.

#### 6. Обробка та аналіз:

У деяких додатках потрібна автоматична обробка: виявлення облич, OCR (розпізнавання тексту), аналіз кольорів або навіть генерація підписів до фото.

#### 7. Безпека:

Слід дотримуватись правил безпеки, щоб уникнути загроз: виконуваного коду у файлах, XSS-атак чи завантаження шкідливих зображень.

## Передача файлів

Ми додамо можливість завантажувати фотографії для студентів. Для роботи із зображеннями скористаємося пакетом multer. Також розглянемо завантаження зображень на хмарну платформу Cloudinary.

Нижче наведено кілька основних способів включення файлів до тіла HTTP-запиту.
Content-Type: multipart/form-data
multipart/form-data — це тип вмісту, який використовується у веб-запитах для передачі файлів або багаточастинних даних через HTTP. Він дає змогу відправляти файли разом із текстовими даними в одному запиті.

### Основні характеристики multipart/form-data:

1. Багаточастинність (multipart):
   Запит складається з кількох частин, кожна має власні заголовки та тіло. Це дає змогу передавати різнорідні дані (тексти, файли, зображення) в одному запиті.

2. Надсилання файлів:
   Головна перевага — можливість передавати файли через HTTP. Користувач обирає файл на своєму пристрої й надсилає його на сервер через веб-форму або клієнт.

3. Кодування даних:
   Частини розділяються спеціальними межами (boundary), а кожна частина містить метадані (наприклад, ім’я поля, назву файла, тип вмісту).

4. Призначення:
   Найчастіше використовується для форм, де користувачі додають зображення, відео, документи тощо.

5. Параметризація:
   Разом із файлами можна передавати додаткові поля (назва, опис, тип тощо).
   Використання multipart/form-data — стандартний спосіб передавання файлів та змішаних даних у веб-додатках.

## Base64

Іноді замість multipart/form-data використовують звичний для API тип application/json, а зображення кодують за допомогою алгоритму Base64.

## Зауваження:

Передавати зображення у Base64 зручно для простих JSON-запитів або коли немає доступу до multipart/form-data.
Водночас Base64 збільшує розмір даних (орієнтовно на 33%) і навантаження на мережу/парсер, тому для великих файлів краще використовувати multipart/form-data.
Якщо зображення все ж передається в JSON, варто обмежувати максимальний розмір тіла запиту та перевіряти MIME-тип/сигнатуру на бекенді.
...
встановимо і налаштуємо middleware multer для завантаження зображень:
...
Cloudinary — це хмарний сервіс для керування зображеннями та відео. Він дозволяє зберігати, обробляти, оптимізувати та доставляти медіафайли. Cloudinary забезпечує можливості завантаження, масштабування, перетворення формату, покращення якості та інтеграцію з іншими веб сервісами для ефективного використання медіаконтенту у веб- та мобільних додатках.

Ми будемо використовувати Cloudinary для завантаження, зберігання та отримання безпечних URL-адрес для медіафайлів.
Реєстрація: Для початку потрібно зареєструватися у Cloudinary
